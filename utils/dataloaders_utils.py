...
def get_exp_dataloader(task):
    ...
    try:
        ...
        print_rank(
            "Dataloader not found, please make sure is located inside the experiment folder"
        )
...
def make_train_dataloader(
    data_config, data_path, clientx, task=None, vec_size=300, data_strct=None
):
    ...
    if clientx is None:
        if (
            not "train_data_server" in data_config
            or data_config["train_data_server"] is None
        ):
    ...
    # Training list on a client side
        ...
        if tokenizer_type != "not_applicable":
            assert clientx >= 0 and clientx < len(
                data_config["train_data"]
            ), "Invalid client index {}".format(clientx)
...
def make_val_dataloader(
    data_config, data_path, task=None, data_strct=None, train_mode=False
):
    """Return a data loader for a validation set"""

    DataLoader = get_exp_dataloader(task)
    val_file = (
        os.path.join(data_path, data_config["val_data"])
        if data_config["val_data"] != None and data_path != None
        else None
    )
    val_dataloader = DataLoader(
        data=data_strct if data_strct is not None else val_file,
        user_idx=0,
        mode="val",
        args=data_config,
    )
...
def make_test_dataloader(data_config, data_path, task=None, data_strct=None):
    ...
    DataLoader = get_exp_dataloader(task)
    test_file = (
        os.path.join(data_path, data_config["test_data"])
        if data_config["test_data"] != None and data_path != None
        else None
    )
    test_dataloader = DataLoader(
        data=data_strct if data_strct is not None else test_file,
        user_idx=0,
        mode="test",
        args=data_config,
    )
