...
class Client:
    ...
    def get_train_dataset(data_path, client_train_config, task):
        ...
        if task == "semisupervision":
            trainset_unlab = get_dataset(
                data_path, client_train_config, task, mode="train", user_idx=-2
            )
            trainset_unlab_rand = get_dataset(
                data_path, client_train_config, task, mode="train", user_idx=-3
            )
        else:
            trainset_unlab = None
    ...
    def get_data(clients, dataset):
        ...
        if dataset == None:  # Training case
            datasets = (
                [train_dataset, trainset_unlab, trainset_unlab_rand]
                if trainset_unlab != None
                else [train_dataset]
            )
        ...
        strcts = (
            []
        )  # Returning list length will always be 1 except when the task is semisupervision
    ...
    def run_testvalidate(client_data, server_data, mode, model):
        ...
        dataloader = None
        print_rank(
            "making dataloader with task {}".format(config["server_config"]["task"]),
        ...
        if mode == "test":
            dataloader = make_test_dataloader(
                data_config,
                data_path=None,
                task=config["server_config"]["task"],
                data_strct=data_strct,
            )
        elif mode == "val":
            dataloader = make_val_dataloader(
                data_config,
                data_path=None,
                task=config["server_config"]["task"],
                data_strct=data_strct,
        ...
        if config["server_config"]["type"] == "personalization":
            ...
            if os.path.exists(local_model_name):
                ...
                if os.path.exists(local_alpha_name):
                    ...
                    print_rank(
                        "Loading Alpha Weight from {}: Value={}".format(
                            local_model_name, alpha
                        )
                    )
                    ...
                    output_local, local_metrics = run_validation_generic(
                        local_model, dataloader
                    )
    ...
    def process_round(client_data, server_data, model, data_path, eps=1e-7):
        ...
        train_dataloader = make_train_dataloader(
            data_config, data_path, task=task, clientx=0, data_strct=data_strct
        )
        ...
        if "updatable_names" in trainer_config:
            set_component_wise_lr(
                model,
                client_config["optimizer_config"],
                trainer_config["updatable_names"],
            )
        ...
        if "ss_config" in client_config and client_config["ss_config"] is not None:
            ss_scheduler = ScheduledSamplingScheduler(
                model=model, **client_config["ss_config"]
            )
        ...
        trainer = Trainer(
            ...
            max_grad_norm=client_config["data_config"]["train"].get(
                "max_grad_norm", None
            ),
            anneal_config=client_config["annealing_config"]
            if "annealing_config" in client_config
            else None,
            num_skips_threshold=client_config["num_skips_threshold"]
            if "num_skips_threshold" in client_config
            else -1,
        ...
        assert (
            "desired_max_samples" in client_config["data_config"]["train"]
        ), "Missing 'desired_max_samples' entry in data config parameter"
        desired_max_samples = client_config["data_config"]["train"][
            "desired_max_samples"
        ]
        ...
        print_rank(
            f'Client setup cost {client_stats["setup"]}s', loglevel=logging.DEBUG
        )
        begin_training = end
        ...
        apply_privacy_metrics = (
            False
            if privacy_metrics_config is None
            else privacy_metrics_config["apply_metrics"]
        )
        ...
        if config["server_config"]["type"] == "personalization":
            ...
            local_optimizer = make_optimizer(
                client_config["optimizer_config"], local_model
            )
            ...
            if os.path.exists(local_model_name):
                ...
                local_trainer.load(
                    ...
                    update_ss_scheduler=False,
                )

            if os.path.exists(local_alpha_name):
                print_rank(
                    "Loading Alpha Weight .. {}".format(local_model_name),
                    loglevel=logging.INFO,
                )
                alpha = torch.load(local_alpha_name)
            ...
            print_rank(
                "client={}, user:{}: LOCAL training loss={}".format(
                    client_id[0], user, train_loss
                ),
                loglevel=logging.INFO,
            )

            local_trainer.save(model_path=model_path, config=config, token=user)

            # Estimate the pseudo-gradient for local model
            for p, orig_param in zip(
                local_trainer.model.parameters(), original_local_model.parameters()
            ):
                orig_param = (
                    orig_param.cuda() if torch.cuda.is_available() else orig_param
                )
        ...
        if privacy_metrics_config and privacy_metrics_config["apply_metrics"]:
            ...
            gradients = extensions.privacy.unroll_network(
                model.named_parameters(), select_grad=True
            )[0]

            if privacy_metrics_config["apply_indices_extraction"]:
                allowed_word_rank = privacy_metrics_config.get(
                ...
                overlap, indices = privacy_metrics.extract_indices_from_embeddings(
                    gradients, batches, embed_dim, vocab_size
                )

                max_overlap = privacy_metrics_config.get("max_allowed_overlap", None)
                if max_overlap is not None and overlap > max_overlap:
                    print_rank(
                        "Removing this client because we extracted {}% words and the maximum allowed is {}%".format(
                ...
                privacy_stats[
                    "Words percentage above " + str(allowed_word_rank) + " word rank"
                ] = ((indices > allowed_word_rank).mean() if len(indices) > 0 else 0)
            ...
            if privacy_metrics_config["apply_leakage_metric"]:
                ...
                orig_params = {
                    n: p
                    for (n, _), p in zip(
                        trainer.model.named_parameters(), model_parameters
                    )
                }
                ...
                leakage = privacy_metrics.practical_epsilon_leakage(
                    orig_params,
                    trainer.model,
                    batches,
                    is_leakage_weighted,
                ...
                print_rank(
                    "privacy leakage: {}".format(leakage), loglevel=logging.DEBUG
                )

                max_leakage = privacy_metrics_config.get("max_allowed_leakage", None)
                if max_leakage is not None and leakage > max_leakage:
                    print_rank(
                        "Removing this client because the information leakage/practical epsilon is {} and the maximum allowed is {}".format(
                            leakage, max_leakage
                        )
                    )
                    client_output["wt"] = 0.0
