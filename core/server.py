...
class OptimizationServer(federated.Server):
    def __init__(
        ...
        self.list_of_train_data = config["client_config"]["data_config"]["train"][
            "list_of_train_data"
        ]
        ...
        if "train" in server_config["data_config"]:
            max_grad_norm = server_config["data_config"]["train"].get(
                "max_grad_norm", None
            )
        else:
            max_grad_norm = None
        ...
        if server_train_dataloader is not None:
            assert (
                "server_replay_config" in server_config
            ), "server_replay_config is not set"
            assert (
                "optimizer_config" in server_config["server_replay_config"]
            ), "server-side replay training optimizer is not set"
            self.server_optimizer_config = server_config["server_replay_config"][
                "optimizer_config"
            ]
            self.server_trainer_config = server_config["server_replay_config"].get(
                "trainer_config", {}
            )
            self.server_replay_iterations = server_config["server_replay_config"][
                "server_iterations"
            ]
        ...
        self.best_model_path = os.path.join(
            self.model_path, "best_val_{}_model.tar".format(self.best_model_criterion)
        )
        ...
        print_rank(
            f"Server successfully instantiated strategy {self.strategy}",
    ...
    def load_saved_status(self):
        ...
        if os.path.exists(self.last_model_path):
            ...
            self.worker_trainer.load(
                self.last_model_path, update_lr_scheduler=True, update_ss_scheduler=True
            )
            if self.server_trainer is not None:
                self.server_trainer.model = (
                    self.worker_trainer.model
                )  # make sure that the models are in sync

        # Check if log is on disk, if so loads it onto current stats
        if os.path.exists(self.log_path):
            with open(
                self.log_path, "r"
            ) as logfp:  # loading the iteration no., best loss and CER
                ...
                self.metrics["best_test_loss"] = elems.get(
        ...
        try:
            ...
            for i in range(self.cur_iter_no, self.max_iteration):
                ...
                if self.quant_thresh is not None:
                    self.config["client_config"]["quant_thresh"] *= self.config[
                        "client_config"
                    ].get("quant_anneal", 1.0)
                    self.quant_thresh = self.config["client_config"]["quant_thresh"]
                    log_metric(
                        "Quantization Thresh.",
                        self.config["client_config"]["quant_thresh"],
                    )

                #  Create the pool of clients -- sample from this pool to assign to workers
                sampled_idx_clients = (
                    random.sample(self.client_idx_list, num_clients_curr_iter)
                    if num_clients_curr_iter > 0
                    else self.client_idx_list
                )
                ...
                apply_privacy_metrics = (
                    self.config.get("privacy_metrics_config", None)
                    and self.config["privacy_metrics_config"]["apply_metrics"]
                )
                adaptive_leakage = apply_privacy_metrics and self.config[
                    "privacy_metrics_config"
                ].get("adaptive_leakage_threshold", None)
                ...
                print_rank(
                    f"Clients sampled from server {sampled_idx_clients}",
                    loglevel=logging.DEBUG,
                )
                for client_output in self.process_clients(
                    sampled_idx_clients, server_data, self.single_worker
                ):
                    ...
                    self.run_stats["communicationCosts"][-1].append(
                        time.time() - client_timestamp
                    )

                    # Get actual pseudo-gradients for aggregation
                    payload_processed = self.strategy.process_individual_payload(
                        self.worker_trainer, client_payload
                    )
                    ...
                    self.run_stats["secsPerClientFull"][-1].append(
                        client_stats["full cost"]
                    )
                    self.run_stats["secsPerClientTraining"][-1].append(
                        client_stats["training"]
                    )
                    self.run_stats["secsPerClientSetup"][-1].append(
                        client_stats["setup"]
                    )
                    self.run_stats["secsPerClient"][-1].append(
                        client_end - clients_begin
                    )
                ...
                if dump_norm_stats:
                    with open(
                        os.path.join(self.model_path, "norm_stats.txt"),
                        "a",
                        encoding="utf-8",
                    ) as outF:
                ...
                if type(adaptive_leakage) is float:
                    ...
                    new_threshold = list(sorted(values))[
                        int(adaptive_leakage * len(values))
                    ]
                    print_rank("Updating leakage threshold to {}".format(new_threshold))
                    self.config["privacy_metrics_config"][
                        "max_allowed_leakage"
                    ] = new_threshold
                ...
                if self.server_trainer is not None:
                    ...
                    self.server_trainer.train_desired_samples(
                        self.server_replay_iterations
                    )
                    self.worker_trainer.model.load_state_dict(
                        self.server_trainer.model.state_dict()
                    )
                ...
                if "val" in eval_list:
                    ...
                    if not (self.losses[0] < self.metrics["best_val_loss"]):
                        ...
                        print_rank(
                            "LOG: Client weight of learning rate {}..".format(
                                self.lr_weight
                            )
                        )
                ...
                self.run_stats["secsPerRoundTotal"].append(
                    self.run_stats["secsPerClientRound"][-1]
                ...
                if self.do_profiling:
                    ...
                    for metric in metrics_for_stats:
                        ...
                        log_metric(
                            f"{metric}Median", np.median(self.run_stats[metric][-1])
                        )
                    ...
                    for k in self.run_stats:
                        if k in metrics_for_stats:
                            print_rank(
                                "{}: {}".format(k, max(self.run_stats[k][-1])),
                                loglevel=logging.DEBUG,
                            )
                        else:
                            print_rank(
                                "{}: {}".format(k, self.run_stats[k][-1]),
                                loglevel=logging.DEBUG,
                            )
