...
def encode_string(word, string_to_int=True):
    ...
    if string_to_int:  # encode
        word = (
            word.ljust(8, " ") if len(word) < 8 else word
        )  # padding -- 8 is max length, all tensors must have the same size during communication
...
def _send_metrics(output):
    ...
    higher_is_better = [
        int(output[key]["higher_is_better"]) for key in output.keys()
    ]  # send the boolean as int
...
def _send_train_output(output):
    ...
    cs_values = [
        float(cs_v) for cs_v in output["cs"].values()
    ]  # cs dict -- values are flatten in 1d array
    pl_values = [float(output["pl"]["weight"])]  # pl dict
    gradients = output["pl"]["gradients"]  # gradients are sent independently
    values = (
        cs_values
        + [float(output[key]) for key in output.keys() if key not in ["cs", "pl"]]
        + pl_values
    )  # reorganizing values in the order expected by the Server
...
def build_grads_dict(node):
    ...
    values = [
        0.0 for i in range(11)
    ]  # initializing tensor shape -- 11 is fixed number of keys expected
    ...
    cs_values = [
        {key: values.pop(0) for key in ["setup", "training", "full cost"]}
    ]  # recreating cs dict
    pl_values = [{"weight": values.pop(), "gradients": grads}]  # recreating pl dict
    values_list = (
        cs_values + [values.pop(0) for i in range(7)] + pl_values
    )  # 7 is fixed length for remaining items
...
def build_metrics_dict(node):
    ...
    values_dict = [
        {"value": float(v), "higher_is_better": bool(higher_is_better[i])}
...
def receive_workers_output(
    node_request_map, results_list, free_nodes, command, idle_nodes
):
    ...
    if dist.get_backend() == "nccl":  # Async
        for node, req in node_request_map:
            if req.is_completed():
                result = (
                    build_metrics_dict(node)
                    if command == COMMAND_TESTVAL
                    else build_grads_dict(node)
                )
                results_list.append(result)
                free_nodes.append(node)
                node_request_map.remove((node, req))
                print_rank(
                    f"Finished releasing the nodes {free_nodes}", loglevel=logging.DEBUG
                )
    else:  # Sync
        ...
        print_rank(
            f" All workers have finished ... taking the remaining clients {len(output)}",
            loglevel=logging.DEBUG,
        )
        output = [
            e for i, e in enumerate(output) if i not in idle_nodes
        ]  # Cleanup for idle workers
...
class Server:
    ...
    def dispatch_clients(
        clients, server_data, command, mode=None, do_profiling=False, single_worker=None
    ):
        ...
        if not single_gpu:
            for worker_rank in range(1, size()):
                ...
                print_rank(
                    f"Finished sending server_data to workers", loglevel=logging.DEBUG
                )
            ...
            while client_queue:
                ...
                _send(
                    command, node
                )  # The command should indicate the worker which function to run on the client
                ...
                print_rank(
                    f"Finished assigning worker {node}, free nodes {free_nodes}",
                    loglevel=logging.DEBUG,
                )
                ...
                if not free_nodes:
                    print_rank(
                        f"Waiting for a workers, free nodes {free_nodes}, reqs_lst {node_request_map}",
                        loglevel=logging.DEBUG,
                    )
                    while len(free_nodes) == 0:
                        (
            ...
            while (len(node_request_map)) != 0:
                node_request_map, results_list, free_nodes = receive_workers_output(
                    node_request_map, results_list, free_nodes, command, idle_nodes
                )
    ...
    def process_clients(clients, server_data, single_worker):
        ...
        return Server.dispatch_clients(
            clients, server_data, COMMAND_TRAIN, single_worker=single_worker
        )
    ...
    def process_testvalidate(clients, server_data, mode, single_worker):
        ...
        return Server.dispatch_clients(
            clients, server_data, COMMAND_TESTVAL, mode, single_worker=single_worker
        )
...
class Worker:
    ...
    def run(self):
        ...
        if not single_gpu:
            while True:  # keeps listening for incoming server calls
                ...
                print_rank(
                    f"Command received {command} on worker {rank()}",
                    loglevel=logging.DEBUG,
                )
                ...
                if command == COMMAND_UPDATE:
                    print_rank(
                        f"COMMMAND_UPDATE received {rank()}", loglevel=logging.DEBUG
                    )
                    lr = _recv(lr, 0)
                    ...
                    print_rank(
                        f"COMMMAND_TRAIN received {rank()}", loglevel=logging.DEBUG
                    )
                    ...
                    print_rank(
                        f"Cliend idx received from Server: {client_idx}",
                        loglevel=logging.DEBUG,
                    )
                    ...
                    output = client_to_process.process_round(
                        client_data, server_data, self.model, self.data_path
                    )
                    ...
                    print_rank(
                        f"COMMMAND_TESTVAL received {rank()}", loglevel=logging.DEBUG
                    )
                    ...
                    client_idx = _recv(client_idx)
                    print_rank(
                        f"Client idx received from Server: {client_idx}, {mode}",
                        loglevel=logging.DEBUG,
                    )
                    ...
                    output = client_to_process.run_testvalidate(
                        client_data, server_data, mode, self.model
                    )
                    ...
                    if dist.get_backend() == "nccl":
                        ...
                        metrics["num"] = {
                            "value": float(num_instances),
                            "higher_is_better": False,
                        }
                        output = metrics
                        print_rank(
                            f"Worker {rank()} output {output}", loglevel=logging.DEBUG
                        )
                        ack = to_device(torch.tensor(1))
                        dist.isend(tensor=ack, dst=0)
                        ...
                        dist.all_gather_object(output, gather_objects[rank()])
                        print_rank(
                            f"Worker {rank()} sent output back", loglevel=logging.DEBUG
                        )
                    ...
                    print_rank(
                        f"COMMMAND_TERMINATE received {rank()}", loglevel=logging.DEBUG
                    )
                    ...
                    print_rank(
                        f"COMMMAND_SYNC_NODES received {rank()}", loglevel=logging.DEBUG
                    )
                    ...
                    dist.all_gather_object(output, gather_objects[rank()])
                    print_rank(
                        f"Worker IDLE {rank()} sent dummy output back",
                        loglevel=logging.DEBUG,
                    )
    ...
    def trigger_train(self):
        ...
        # Execute Client.process_round()
        GLOBAL_MESSAGE = client_to_process.process_round(
            client_data, server_data, self.model, self.data_path
        )
