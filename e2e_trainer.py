    ...
    if WORLD_RANK < federated.size() and LOCAL_RANK < LOCAL_RANK_LIMIT:
        ...
        if WORLD_RANK == 0:
            ...
            if federated.size() == 1:
                ...
                single_worker = federated.Worker(
                    model=model,
                    data_path=data_path,
            ...
            val_dataset = get_dataset(
                data_path, config, task, mode="val", test_only=True
            )
            test_dataset = get_dataset(
                data_path, config, task, mode="test", test_only=True
            )
            ...
            if WORLD_RANK == 0:
                try:
                    ...
                    server_setup = select_server(server_type)  # Return the server class
                    server = server_setup(
                        num_clients=config["server_config"]["data_config"][
                            "num_clients"
                        ],
                        model=model,
                        optimizer=optimizer,
